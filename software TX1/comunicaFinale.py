...
while True:
	...
	if "ON" in x and not(oggetto):
	 ...
	 #STOP ARDUINO
		...
		while flag:
			k = 0
			j = [-1,2] # -1 indirizzo della prima camera e 2 indirizzo della seconda camera CAMBIAMENTO JETSON TX1
			for i in range(0,nc):  
				camera[i] = cv2.VideoCapture(j[i])
	...
	#STOP ARDUINO
		...
		for i in range(0,nc): 
			...
			#WB + SAVE
			print 'White Balance'
			cv2.imwrite("immagini_fotocamere/camera_" + str(k) + "_in_SCATTA" + ".png", frame[i])		
			frame[i] = edge(frame[i]);			
			cv2.imwrite("immagini_fotocamere/camera_" + str(k) + "_out_SCATTA" + ".png", frame[i])
			camera[i].release()
			k=k+1
		#3D RECOSTRUCTION
		print '3D recostruction'
